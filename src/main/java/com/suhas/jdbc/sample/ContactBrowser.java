...
public class ContactBrowser extends Application {
	...
	private String[] propertyName = { "id", "name", "nickName", "address", "homePhone", "workPhone", "cellPhone",
			"email", "birthDate", "webSite", "profession" };
	private String[] propertyLabel = { "ID", "Name", "Nick Name", "Address", "Home Phone", "Work Phone", "Cell Phone",
			"Email", "Birth Date", "Website", "Profession" };
	...
	public void start(Stage primaryStage) {
		...
		searchField.textProperty().addListener((observable, oldValue, newValue) -> filteredData.setPredicate(str -> {
		...
		contactTableView.setItems(observableNames);
		...
		listView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<ContactPerson>() {
			@Override
			public void changed(ObservableValue<? extends ContactPerson> observable, ContactPerson oldValue,
